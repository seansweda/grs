...
void frame::frameput(void)
{
int i;
fprintf(output,"UP:%-10s ",onbase[0]->nout());
for (i=1; i<4; i++) 
   fprintf(output,"%1d:%-10s ",i,onbase[i] ? onbase[i]->nout() : "    ");
fprintf(output,"P:%-10s ",pit->mound->nout());
